...
var Browser = function(id, fullName, /* capturedBrowsers */ collection, emitter, socket, timer,
    /* config.browserDisconnectTimeout */ disconnectDelay, /* config.notExecutedRetryLimit */ notExecutedRetryLimit) {
  ...
  this.onStart = function(info) {
    ...
    if (info.total === null) {
      log.warn('Adapter did not reporter total number of specs.');
    }
  ...
  this.onDisconnect = function() {
    if (this.state === READY) {
      if (config.notExecutedRetryLimit && config.notExecutedRetryLimit--) {
         log.warn('Disconnected not tests were executed, retrying...');
         disconnect();
         emitter.emit('browser_no_tests_executed', self);
         return;
      }
      disconnect();
