...
var start = function(injector, config, launcher, globalEmitter, preprocess, fileList, webServer,
  ...
  webServer.on('error', function(e) {
    if (e.code === 'EADDRINUSE') {
      log.warn('Port %d in use', config.serverPort);
      config.port++;
      webServer.listen(config.serverPort, config.serverHost);
    } else {
      throw e;
    }
  });
  ...
  // A map of to keep track of tolerated disconnection by browser.
  var toleratedDisconnectionByBrowser = Object.create(null);
  ...
  webServer.listen(config.serverPort, config.serverHost, function() {
    log.info('Karma v%s server started at http://%s:%s%s', constant.VERSION, config.serverHost
        || constant.DEFAULT_HOSTNAME,
        config.serverPort, config.urlRoot);
  ...
  socketServer.sockets.on('connection', function (socket) {
    ...
    socket.on('register', function(info) {
      ...
      if (config.singleRun) {
        ...
        singleRunBrowsers.add(newBrowser || capturedBrowser);
      }
  ...
  if (config.singleRun) {
    globalEmitter.on('browser_complete', function(completedBrowser) {
      var id = completedBrowser.id;
      singleRunDoneBrowsers[id] = true;

      if (launcher.kill(id)) {
        var disconnectLimit = toleratedDisconnectionByBrowser[id];
        if (isNaN(disconnectLimit)) {
          disconnectLimit = config.disconnectTolerance;
        }

        if (config.singleRun && disconnectLimit > 0 &&
            completedBrowser.state === browser.Browser.STATE_DISCONNECTED) {
          disconnectLimit--;
          log.warn('Retrying %s (%s attempts remaining)', completedBrowser.name,
              disconnectLimit);
          // let's kick out the disconnected browser
          singleRunBrowsers.remove(completedBrowser);
          var launchers = launcher.launch([launcher.getBrowserNameById(id)],
              config.clientHost, config.clientPort, config.urlRoot);
          id = launchers[launchers.length - 1].id;
          singleRunDoneBrowsers[id] = false;
          toleratedDisconnectionByBrowser[id] = disconnectLimit;
        } else {
          // workaround to suppress "disconnect" warning
          completedBrowser.state = browser.Browser.STATE_DISCONNECTED;
        }
      }
