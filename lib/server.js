...
var start = function(injector, config, launcher, globalEmitter, preprocess, fileList, webServer,
  ...
  if (config.singleRun) {
    ...
    globalEmitter.on('browser_no_tests_executed', function(browser) {
        if (launcher.kill(browser.id)) {
            launcher.launch([browser.fullName], config.hostname, config.port, config.urlRoot);
        }
    });
