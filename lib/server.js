...
var start = function(injector, config, launcher, globalEmitter, preprocess, fileList, webServer,
  ...
  // A map of disconnect tolerances by browser.
  var disconnectTolerances = Object.create(null);
  ...
  if (config.singleRun) {
    globalEmitter.on('browser_complete', function(completedBrowser) {
      ...
      if (launcher.kill(id)) {
        ...
        var disconnectTolerance = (disconnectTolerances[id] || config.disconnectTolerance) + 0;

        if (disconnectTolerance && completedBrowser.state === browser.Browser.STATE_DISCONNECTED) {
            disconnectTolerance--;
            var message = completedBrowser.name + ': Retrying (' + disconnectTolerance + ' attempts remaining)';
            log.warn(message);
            // let's kick out the disconnected browser
            singleRunBrowsers.remove(completedBrowser);
            var browsers = [launcher.getNameById(id)];
            var launchers = launcher.launch(browsers, config.hostname, config.port, config.urlRoot);

            id = launchers[launchers.length - 1].id;
            singleRunDoneBrowsers[id] = false;
            disconnectTolerances[id] = disconnectTolerance;
