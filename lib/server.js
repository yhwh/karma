  ...
  if (config.singleRun) {
    globalEmitter.on('browser_complete', function(completedBrowser) {
      ...
      if (launcher.kill(completedBrowser.id)) {
        ...
        var disconnectTolerance = disconnectTolerances[completedBrowser.id] || config.disconnectTolerance;
        ...
        if (disconnectTolerance && completedBrowser.state === browser.Browser.STATE_DISCONNECTED) {
          ...
          var browsers = [launcher.getNameById(completedBrowser.id)];
          delete singleRunDoneBrowsers[completedBrowser.id];
          singleRunBrowsers.remove(completedBrowser);
          launcher.launch(browsers, config.hostname, config.port, config.urlRoot)
            .forEach(function(launchedBrowser) {
              var id = launchedBrowser.id;
