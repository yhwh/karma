...
var createKarmaMiddleware = function(filesPromise, serveStaticFile,
  ...
  return function(request, response, next) {
    ...
    if (/^http[s]?:\/\/([a-z\.\:\d]+)\//.test(requestUrl)) {
      requestUrl = requestUrl.replace(/^http[s]?:\/\/([a-z\.\:\d]+)\//, '/');
    }

    // redirect /__karma__ to /__karma__ (trailing slash)
    if (requestUrl === urlRoot.substr(0, urlRoot.length - 1)) {
      var moved = 'MOVED PERMANENTLY';
      response.writeHead(301, {
        'Content-Type': 'text/plain',
        'Content-Length': moved.length,
        'Location': urlRoot
      });
      return response.end(moved);
    }
