...
var normalizeConfig = function(config, configFilePath) {
  ...
  if (!config.singleRun && config.disconnectTolerance) {
    log.debug('disconnectTolerance set to 0, because of singleRun');
    config.disconnectTolerance = 0;
  }
...
var Config = function() {
  ...
  // DEFAULT CONFIG
  this.frameworks = [];
  this.clientPort = constant.DEFAULT_PORT;
  this.clientHost = constant.DEFAULT_HOSTNAME;
  this.serverHost = null;
  this.serverPort = constant.DEFAULT_PORT;
  this.basePath = '';
  this.files = [];
  this.exclude = [];
